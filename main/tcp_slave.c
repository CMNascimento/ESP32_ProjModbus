...
static void variable_update(void);
...
static void setup_reg_data(void)
{
    ...
    holding_reg_params.holding_data1 = 0x00;
    ...
    // Read
    input_reg_params.input_data0 = 0.0; 
    input_reg_params.input_data1 = 0x0000; 
...
static esp_err_t slave_init(mb_communication_info_t* comm_info)
{
    ...
    printf("sizeof(coil_reg_params): %d", sizeof(coil_reg_params));
    reg_area.size = 1;//sizeof(coil_reg_params);
    err = mbc_slave_set_descriptor(reg_area);
    MB_RETURN_ON_FALSE((err == ESP_OK), ESP_ERR_INVALID_STATE,
                                    TAG,
                                    "mbc_slave_set_descriptor fail, returns(0x%x).",
...
static void variable_update(void){
    ...
    input_reg_params.input_data0 = voltage;

    if (coil_reg_params.coils_port0 == 0)
    {
        led_state = 0;
    }
    else if(coil_reg_params.coils_port0 == 1){
        led_state = 1;
    ...
    discrete_reg_params.discrete_input0 = led_state;
